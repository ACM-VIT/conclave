...
export function useMeetMedia({
  ...
  const stopManagedCameraTrack = useCallback(
    (managedTrack?: ManagedCameraTrack | null) => {
      if (!managedTrack) return;
      intentionalTrackStopsRef.current.add(managedTrack.track);
      if (managedCameraTrackRef.current?.track.id === managedTrack.track.id) {
        managedCameraTrackRef.current = null;
      }
      managedTrack.stop();
    },
    ...
    try {
      ...
      if (!isCameraOff) {
        try {
          ...
          stopManagedCameraTrack(previousManagedTrack);
  ...
  const updateVideoQuality = useCallback(
    async (quality: VideoQuality) => {
      ...
      let nextManagedTrack: ManagedCameraTrack | null = null;

      try {
        console.log(`[Meets] Switching to ${quality} quality`);
        nextManagedTrack = await createManagedCameraTrackForCurrentSettings();
        ...
        if (previousVideoTrack) {
          stopLocalTrack(previousVideoTrack);
        }
        stopManagedCameraTrack(previousManagedTrack);
      } catch (err) {
        console.error("[Meets] Failed to update video quality:", err);
        stopManagedCameraTrack(nextManagedTrack);
        managedCameraTrackRef.current = previousManagedTrack;
        videoProducerRef.current = previousProducer;
        if (previousVideoTrack) {
          setLocalStream((prev) => {
            const remainingTracks =
              prev?.getTracks().filter((track) => track.kind !== "video") ?? [];
  ...
  const toggleCamera = useCallback(async () => {
    ...
    if (isCameraOff) {
      ...
      try {
        ...
        stopManagedCameraTrack(createdManagedTrack);
        if (createdTrack) {
          stopLocalTrack(createdTrack);
          setLocalStream((prev) => {
            if (!prev) return prev;
