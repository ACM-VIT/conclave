...
function JoinScreen({
  ...
  const [authBackPhase, setAuthBackPhase] = useState<"welcome" | "join">(
    hasUserIdentity ? "join" : "welcome",
  );
  const lastNonAuthPhaseRef = useRef<"welcome" | "join">(
    hasUserIdentity ? "join" : "welcome",
  );
  ...
  useEffect(() => {
    ...
    lastNonAuthPhaseRef.current = phase;
  }, [phase]);

  const prevAuthTokenRef = useRef(authRequestToken);

  useEffect(() => {
    if (!authRequestToken) return;
    if (authRequestToken === prevAuthTokenRef.current) return;

    prevAuthTokenRef.current = authRequestToken;
    setAuthBackPhase(lastNonAuthPhaseRef.current);
    setPhase("auth");
  }, [authRequestToken]);
          ...
          <div className="w-full max-w-sm animate-slide-up">
            ...
            <div className="grid gap-3 sm:grid-cols-2 mb-3">
              ...
              >
                ...
                <span
                  className="text-[13px] sm:text-sm leading-none whitespace-nowrap tracking-tight"
                  style={{ fontFamily: "'PolySans Trial', sans-serif" }}
                >
            ...
            <button
              onClick={() => setPhase(authBackPhase)}
            ...
            <div className="flex flex-col lg:pr-2">
              <div className="relative aspect-video lg:aspect-[16/10] bg-[#0d0e0d] rounded-2xl overflow-hidden border border-[#FEFCD9]/10 shadow-2xl">
                {isCameraOn && localStream ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-[#1a1a1a] to-[#0d0e0d]">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#F95F4A]/20 to-[#FF007A]/20 border border-[#FEFCD9]/20 flex items-center justify-center">
                      <span className="text-3xl text-[#FEFCD9] font-bold">
                ...
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/50 backdrop-blur-sm rounded-full px-2 py-1.5">
                  <button
                    onClick={toggleMic}
                    className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${isMicOn ? "text-[#FEFCD9] hover:bg-white/10" : "bg-red-500 text-white"}`}
                  >
                    {isMicOn ? (
                  ...
                  <button
                    ...
                    className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${isCameraOn ? "text-[#FEFCD9] hover:bg-white/10" : "bg-red-500 text-white"}`}
                  >
                    {isCameraOn ? (
                  ...
                  <div className="space-y-4">
                    {isAdmin && allowGhostMode && (
                      <>
                        <div>
                          <label
                            className="text-[10px] uppercase tracking-wider text-[#FEFCD9]/40 mb-1.5 block"
                            style={{ fontFamily: "'PolySans Mono', monospace" }}
                          >
                            Display Name
                          </label>
                          <input
                            ...
                            onChange={(e) =>
                              onDisplayNameInputChange(e.target.value)
                            }
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          maxLength={
                            enforceShortCode ? ROOM_CODE_MAX_LENGTH : undefined
                          }
                          disabled={isLoading}
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-[#0d0e0d]/80 flex items-center justify-center z-50">
          <div className="flex flex-col items-center gap-3">
            ...
            <span
              className="text-sm text-[#FEFCD9]/60"
              style={{ fontFamily: "'PolySans Mono', monospace" }}
            >
